...
app.post("/jokes", (req, res) => {
  ...
  jokes.push(joke);
  console.log(jokes[jokes.length - 1]);  // console.log(jokes.slice(-1));
  res.json(joke);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === idnew);
  jokes[searchIndex] = newjoke;
  res.json(newjoke);  
  // jokes[idnew-1] = newjoke; // that is assuming joke id is in order
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const newjoke = {
    id: id,
    jokeText: req.body.text || existingJoke.jokeText, // checks if req.body.text exists and chooses it. or operator ignores existing one.
